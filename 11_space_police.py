...
def move(codes, is_origin_white = False):
    ...
    intcode = Intcode(0, codes, [1, 2, 3, 4, 5, 6, 7, 8, 9, 99])

    while not intcode.halted():
        inst = [1] if pos in white_pos else [0]

        for _ in range(0, 2):
            intcode.intcode(inst)
        ...
        direction = right(direction[0], direction[1]) if turn == 1 else left(direction[0], direction[1])
        pos = sum(i[0] for i in [pos, direction]), sum(i[1] for i in [pos, direction])
...
if __name__ == '__main__':
    # Part 1
    visited, _ = move(puzzle_input)
    print(len(visited))

    # Part 2
    _, white_pos = move(puzzle_input, True)
    h = [i[0] for i in white_pos]
    w = [i[1] for i in white_pos]

    for y in range(min(h), max(h) + 1):
        message  = ''
        for x in range(min(w), max(w) + 1):
            message += 'X' if (y, x) in white_pos else ' '
        print(message)
